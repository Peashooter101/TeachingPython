...
For Loops
-----
A little backwards to teach While and then For loops, but we needed the foundational ideas for loops,
since Python does for loops a little differently.

Most for loops you will use in Python are like "for-each"
loops. Each value in a given group of elements will be executed.
"""

# range() is a method that adds number counting similar to what we had for the while loop.
# They will produce integers from the minimum up to less than the maximum, incremented by some given value.
...
#
# "for" will set up the entire loop and put the value into an iterator variable that you designate.
#
# So... With all that information, consider the following code...

for i in range(10):
    print("For Loop's i is", i)
...
for i in range(5):
    ...
    i = 100
    print("Reassigned value of i to", i)
...
# Normally, without the condition statements, the output would count the values from 1 through 10.
# With the conditionals, all even numbers will continue the loop without outputting the value
# and 10 does not even get shown because the loop is broken when i == 9.
#
...
Summary
-----
For loops are blocks of code that keep executing so long as there is still more things to execute off of.
    for variable in iterable:
    
You can use the variable's name to access the current element for that iteration of the loop.
    for varName in iterable:
        print(varName)
    
Something iterable includes range() which can be used to produce numbers for the loop.
