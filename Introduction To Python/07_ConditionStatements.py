...
Condition Statements are statements and blocks of code
that can execute if a condition is True. They rely
on the usage of Comparison and Logical Operators to work,
both of which output Boolean values.
"""

# Code blocks are designated by their indentation, so indentation
# in Python is INSANELY important. Do not screw up your indentation.
#
...
# Your IDE will be highlighting the second print statement,
# saying it is wrong because the code is unreachable.
# It is unreachable because the condition is always false.
# An if statement will only execute the code block if
# the condition is True.

# You can chain these if statements together using
...
# Let's put everything together. Here is a simple program
# that will ask for a number from the user. If it is larger
# than 50, say it's big. If it is not, but it is larger than 25,
